package com.example.spring09.model.memo.dao;
...
public interface MemoDAO {
	...
	@Select("select * from memo1 order by idx desc")
	public List<MemoDTO> list();//import java.util.List;
	
	//mybatis query에 전달할 변수는 @Param으로 처리
	@Insert("insert into memo1 (idx,writer,memo) values "
			+ "( (select nvl(max(idx)+1,1) from memo1)"
			+ ", #{writer}, #{memo} )")
